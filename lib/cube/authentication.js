    ...
    var userRecorder = function() {
      ...
      try {
        ...
        auth_ok({
          ...
          roleId: payload.context.user.roleId,
...
// records user info without validating signature
authentication.trusted_recording_signed_request = function(db, options) {
  var signedRequest = require('./signed-request');
  var userRecorder = function(request, auth_ok, auth_no) {
    metalog.event('cube_auth', { authenticator: 'trusted_recording_signed_request', path: request.url, method: request.method }, 'minor');
    if (request.method === 'OPTIONS') {
      return auth_ok({ anonymous: true});
    }
    var authHeader = request.headers.authorization;
...
authentication.orgid_restricted_signed_request = function(db, options) {
  ...
  var check = function(request, auth_ok, auth_no) {
    var orgIdChecker = function() {
      ...
      try {
        ...
        var orgIds = extractOrgId(expression);

        if (! orgIds.length) {
          return auth_no('An orgId filter is required');
        }

        orgIdFilter = orgIds[0];
        for (var i = 1, c = orgIds.length; i < c; i++) {
          if (orgIds[i] === null) {continue;}
          if (orgIds[i] !== orgIdFilter) {
            return auth_no('Only one unique orgId allowed in an expression');
          }
        }
      } catch (e) {
        console.warn(e.stack || e);
        return auth_no('Invalid expression');
      }
  ...
  function extractOrgId(expression) {
    // Handle complex expressions
    if (expression.left) {
      return extractOrgId(expression.left).concat(extractOrgId(expression.right));
    }

    // Handle non-expression segments (eg. numbers)
    if (expression.value && ! expression.filter && ! expression.fields && ! expression.exists) {
      return [ null ];
    }
