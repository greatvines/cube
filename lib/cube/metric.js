...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    measure(expression, start, stop, tier, request, "id" in request
  ...
  function measure(expression, start, stop, tier, request, callback) {
    (expression.op ? binary : expression.type ? unary : constant)(expression, start, stop, tier, request, callback);
  }

  // Computes a constant expression;
  function constant(expression, start, stop, tier, request, callback) {
  ...
  function unary(expression, start, stop, tier, request, callback) {
    var remaining = 0,
        time0 = Date.now(),
        time = start,
  ...
  function findOrComputeUnary(expression, start, stop, tier, request, callback) {
    ...
    if (request.orgId) {
      filter['d.user.orgId'] = request.orgId;
    }
  ...
  function binary(expression, start, stop, tier, request, callback) {
    var left = {}, right = {};
