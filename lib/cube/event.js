...
exports.putter = function(db) {
  ...
  function save(type, event) {
    ...
    saveLatest(type, event);
    queueInvalidation(type, event);
  }

  function latestQuery(type, event) {
    if (!(event.d && event.d.user && event.d.event)) {
      return;
    }
    return {
      "d.user.username": event.d.user.username,
      "d.user.orgId": event.d.user.orgId,
      "d.event": event.d.event
    };
  }

  function saveLatest(type, event) {
    var query = latestQuery(type, event);
    if (!query) {
      return;
    }
    collection(type).latest.findAndModify(
      query,
      [['_id','asc']],
      _(event).omit('_id'),
      { upsert: true },
...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    var stream = ! (("stop" in request) || ("aggregate" in request)),
    ...
    if (request.aggregate) {
      ...
      // If we are not given an specific aggregate query, build one using the defaults
      // and more specific query params
      if (request.aggregate === '1') {
        query.push({ $match: filter });

        // Write the sort clause
        try {
          var sort = JSON.parse(request.sort);
        } catch (err) {
          sort = null;
        }
        if (sort !== false) {
        ...
        try {
          var group = JSON.parse(request.group);
        } catch (err) {
          group = null;
        }
        if (group !== false) {
      ...
      // If we are given a specfic aggregate query, use it
      else {
        query.push({ $match: filter });

        // Pull out the unneeded $exists clauses
        Object.keys(filter).forEach(function(key) {
          if (typeof filter[key] === 'object') {
            var keys = Object.keys(filter[key]);
            if (keys.length === 1 && keys[0] === '$exists') {
              delete filter[key];
            } else {
              delete filter[key].$exists;
            }
          }
        });

        var aggregate = JSON.parse(request.aggregate);
        if (! Array.isArray(aggregate)) {
          aggregate = [ aggregate ];
        }
        query.push.apply(query, aggregate);
      }

      // Log the query
      console.log(require('util').inspect(query, {depth: 5, colors: true}));

      // Run the query and prepare results
      return collection(expression.type).events.aggregate(query, {
        allowDiskUse: true
      }, function(err, result) {
        handle(err);
